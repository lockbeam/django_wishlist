...
class TestAddNewPlace(TestCase):
    ...
    def test_add_new_unvisited_place(self):
        ...
        response_places = response.content['places']
        self.assertEqual(1, len(response_places)) # to check only one place
        tokyo_from_response = response_places[0]

        #so check if Tokyo is in the database:
...
class TestVisitPlace(TestCase):
    ...
    def test_non_existent_place(self):
        visit_nonexistent_place_url = reverse('place_was_visited', args=(24393289, ))
